import * as express from 'express';
import * as bodyParser from 'body-parser';
import content_services from '../content/content-services';
import { Configuration } from '../model/Configuration';

class TogoContentServer {
    public app:express.Application;

    constructor(){
        this.app = express();
        this.config();
    }

    config(){
        const configuration = this.getConfiguration()
        this.app.use(bodyParser.urlencoded());
        this.app.use(bodyParser.json());
        content_services(this.app,configuration);
    }

    private getConfiguration():Configuration{
        return {
            authority:'https://login.microsoftonline.com',
            tenant:'everistogodev.onmicrosoft.com',
            clientId:'e72d88b7-3e18-4b9a-96dd-5ef1a28babb9',
            secretId:'6B2bDdSTtWf8k8tqNyRKfu3nFk0vrQxJFfSEC7jIRQ=',
            publicKey:'',
            resources: {
                sharepoint:'https://everistogodev.sharepoint.com',
                graph:''
            }
        }
    }
}

export default new TogoContentServer().app;